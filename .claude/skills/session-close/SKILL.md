...
3. `2_지식/open-loops.md` 갱신 — **삭제는 이중 주체**: ①Claude가 이번 세션에서 종결을 확인한 항목 ②사용자가 `- [x]` 로 체크한 항목(Claude가 종결을 못 알아챈 것을 사용자가 알려주는 채널). 둘 중 하나면 하위 불릿까지 통째로 삭제. **확신 없으면 지우지 말고 보고**(원문이 링크 노트에 남아있는지 확인 후 삭제). 새 미결은 `- [ ]` 로 추가. "다음 세션" 기입.
